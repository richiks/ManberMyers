 ...
 * @date 10/28/2019
 * @brief Implementation of the the DC3 algorithm or the skew algorithm.
 */
...
namespace {
  ...
  /* Given a string, forms the intermediate DC3 doubled string.
  ...
  vector<size_t> doubledVersionOf(const vector<size_t>& str) {
    ...
    /* Copy over the string starting at the first (zero-indexed) position, then
     * pad with sentinel characters up to a multiple of three.
     */
    ...
    /* Copy over the string starting at the second (zero-indexed) position and
     * pad with sentinels up to a length that's a multiple of three.
     */
  ...
  SuffixArray solveNonCongruentPositionsIn(const vector<size_t>& str) {
    ...
    /* Radix sort these items to get them in order. The number of buckets we'll
     * need is equal to the maximum value in the new string plus one (because
     * values range from 0 up to the maximum, inclusive.)
     */
    size_t maxValue = *max_element(bigString.begin(), bigString.end());
    radixSort(blocks, maxValue + 1);

    /* Form a new string by relabeling each character with its index. We have
     * to be careful here because certain blocks might be duplicated and need
     * to get the same label. To account for this, we'll start labeling the
     * blocks with 1 and increment that label whenever we encounter a new
     * block.
     */
    vector<size_t> newString(blocks.size() + 1); // Implicity 0-terminated

    /* Start counting at 1, since we need to put our own terminator character
  ...
  vector<size_t> sortCongruentPositions(const vector<size_t>& str,
    ...
    /* We will be working with suffixes whose positions are multiples of three.
     * Each suffix will be represented as a pair consisting of its first
     * character, followed by the rank of the T12 suffix that comes right after
     * it.
     *
     * For coding simplicity, we've actually represented these blocks as
     * triples whose last component is ignored. That lets us reuse the radix
     * sort implementation from before.
     */
    vector<DC3Block> blocks;
    ...
    /* We again have the suffix array (each position gives the index of the
     * suffix that belongs there), but we want the inverse suffix array (each
     * position gives the index whose rank equals that position.)
     */
    vector<size_t> result;
  ...
  SuffixArray merge(const vector<size_t>& sorted0,
    ...
    /* Indices keeping track of the next unused position in the sorted0 and
     * sorted12 arrays.
     */
    size_t i0 = 0, i12 = 0;
